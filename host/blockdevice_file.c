...
typedef struct {
    ...
    int fildes;
    mutex_t _mutex;
} blockdevice_file_config_t;

static const char DEVICE_NAME[] = "file";

static int init(blockdevice_t *device) {
    (void)device;
    blockdevice_file_config_t *config = device->config;
...
blockdevice_t *blockdevice_file_create(const char *path, size_t capacity, size_t block_size) {
    ...
    blockdevice_file_config_t *config = calloc(1, sizeof(blockdevice_file_config_t));
    if (config == NULL) {
        free(device);
        return NULL;
    }
